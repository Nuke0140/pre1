...
export async function GET(req: NextRequest) {
  ...
  const reports = await ReportService.listCustomReports(ctx)
  ...
  try {
    ...
    if (!payload.name || !payload.source || !payload.fields || payload.fields.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Name, source, and at least one field are required' },
        { status: 400 }
      )
    }
    ...
    const saved = await ReportService.saveCustomReport(payload, ctx)
